...
export const MatchListItem: React.FC<MatchListItemProps> = ({ match, onClick, isFavorite, onToggleFavorite }) => {
  ...
  return (
    <div 
      ...
      className={`bg-gray-800 rounded-lg p-4 cursor-pointer hover:bg-gray-700/70 transition-all duration-200 shadow-lg border ${isFavorite ? 'border-yellow-400/50' : isLive ? 'border-red-500/50' : 'border-transparent hover:border-green-500'}`}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
