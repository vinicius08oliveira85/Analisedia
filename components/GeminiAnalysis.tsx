            ...
            <div className="text-center p-2 sm:p-3">
                ...
                {isLoading && (
                    <div className="flex flex-col items-center justify-center text-gray-300">
                        <svg className="animate-spin h-6 w-6 sm:h-8 sm:w-8 text-green-400 mb-2 sm:mb-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        <p className="font-semibold text-[10px] sm:text-xs">Analisando dados, aguarde...</p>
                        <p className="text-[9px] sm:text-[10px] text-gray-500 mt-0.5 sm:mt-1">Isso pode levar alguns segundos.</p>
                    </div>
                )}
                {error && (
                     <div className="bg-red-900/50 border border-red-700 text-red-300 px-2 py-2 sm:px-3 sm:py-2.5 rounded-lg relative text-left" role="alert">
                        <strong className="font-bold text-[10px] sm:text-xs">Erro! </strong>
                        <span className="block sm:inline text-[10px] sm:text-xs">{error}</span>
                        <button onClick={handleGenerateAnalysis} className="text-[9px] sm:text-[10px] font-semibold underline ml-2 sm:ml-4">Tentar novamente</button>
                    </div>
                )}
                {analysis && (
